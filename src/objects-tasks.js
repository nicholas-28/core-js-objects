...
function shallowCopy(obj) {
  const target = {};
  const copy = Object.assign(target, obj);
  return copy;
}
...
function mergeObjects(objects) {
  const mergedEntries = objects.reduce((acc, obj) => {
...
function makeWord(lettersObject) {
  ...
  Object.values(lettersObject).forEach((positions) => {
    const maxPositions = Math.max(...positions);
    if (maxPositions > maxLength) {
      maxLength = maxPositions;
    }
  });
  const wordArray = new Array(maxLength + 1);
  ...
  const change = queue.reduce(
    (acc, bill) => {
      if (bill === 25) {
        ...
        if (acc[25] === 0) acc.insufficientChange = true;
        acc[25] -= 1;
        acc[50] += 1;
      } else if (bill === 100) {
        if (acc[50] > 0 && acc[25] > 0) {
          acc[50] -= 1;
          acc[25] -= 1;
        } else if (acc[25] >= 3) {
          acc[25] -= 3;
        } else {
          acc.insufficientChange = true;
        }
      }
      return acc;
    },
    { 25: 0, 50: 0, insufficientChange: false }
  );

  return !change.insufficientChange;
}
